...
- Version detection now uses `runtime/debug.ReadBuildInfo()` for correct version when installed via `go install pkg@version`
- CLI `--version` flag now shows actual version instead of "dev" when installed via `go install`
...
[1.0.2]: https://github.com/lex00/cfn-lint-go/releases/tag/v1.0.2
[1.0.0]: https://github.com/lex00/cfn-lint-go/releases/tag/v1.0.0
